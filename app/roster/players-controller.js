  ...
  // this.getPlayersByTeam = function getPlayersByTeam(e) {
    ...
    function updateRoster(players) {
        ...
        for (var i = 0; i < players.length; i++) {
          ...
          <button class="button btn-success" id="${player.id}" onclick="app.controllers.playersController.add('${player.id}')">Add to Team</button>
      </div>
          `
        }
    ...
    function updateMyTeam(players) {
        ...
        for (var i = 0; i < players.length; i++) {
            var player = players[i];
            template += `
          <div class="player-card animated fadeInUp">
          <img src="${player.photo}" alt="">
          <h1>${player.fullname}</h1>
          <h2>${player.position}</h2>
          <h3>${player.pro_team}, ${player.jersey}</h3>
          <button class="button btn-danger" id="${player.id}" onclick="app.controllers.playersController.remove('${player.id}')">Remove From Team</button>
      </div>
          `
